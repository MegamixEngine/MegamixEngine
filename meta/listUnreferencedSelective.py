...
roomsChosen = ["rmInit","lvlCopyThisRoom","lvlGameIntro","rmCastleIntro","rmCharacterSelect","rmCredits","rmDisclaimer","rmFileSelect","rmGameOver","rmNewFile","rmOptions","rmRoomSelect","rmShop","rmStageSelect","rmTitleScreen","rmWeaponGet", "lvlJudgeHUB", "lvlJudgesGoHere", "lvlTeleporter","lvlTutorial_RCRJ","lvlTutorialFlameMixer","lvlShowcaseHUB", "lvlMainHUB"];
...
def check_file_for_references(ref_file, ref_name, names, unprefixed_names, graph):
	...
	#		for arg in sys.argv:
	#			if arg != "--generate-lightweight" and arg != sys.argv[0] and arg != sys.argv[1]:
	#				roomsChosen.append(arg);
	#	if (ref_name not in roomsChosen):
	...
	with open(project_file, "r") as projf:
		...
		for line in s.splitlines(False):
			...
			if match:
				...
				#print(match_name);
				if match_type == "room" and match_name in roomsChosen:
